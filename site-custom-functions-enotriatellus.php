...
function am_add_text_for_specific_product( $order, $sent_to_admin, $plain_text, $email ) {
    ...
    if ( $sent_to_admin || ! isset( $email->id ) || $email->id !== 'customer_processing_order' ) {
        return;
    }
    ...
    $target_product_ids = array( 2186 );

    // Testi (HTML / Plain)
    $text_html  = "<p><strong>Degustazione in cantina - </strong> Ti contatteremo a breve via email per confermare la prenotazione.</p>";
    $text_plain = "Degustazione in cantina - Ti contatteremo a breve via email per confermare la prenotazione.\n\n";
